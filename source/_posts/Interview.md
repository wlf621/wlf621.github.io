...
简体中文
&emsp;&emsp; | &emsp;&emsp;
<a href="https://gitee.com/wlf621/Interview/blob/master/README_en.md">English</a>
</div> 
<br>

<b><details><summary>💡 关于</summary></b>

📚 本仓库是面向 C/C++ 技术方向校招求职者、初学者的基础知识总结，包括语言、程序库、数据结构、算法、系统、网络、链接装载库等知识及面试经验、招聘、内推等信息，转载自https://interview.huihut.com
...
* 内联函数不能是表现多态性时的虚函数，解释见：[虚函数（virtual）可以是内联函数（inline）吗？](https://gitee.com/wlf621/Interview#%E8%99%9A%E5%87%BD%E6%95%B0virtual%E5%8F%AF%E4%BB%A5%E6%98%AF%E5%86%85%E8%81%94%E5%87%BD%E6%95%B0inline%E5%90%97)
...
### [智能指针](https://segmentfault.com/a/1190000016055581)
...
### C++ [多线程编程](https://www.runoob.com/w3cnote/cpp-std-thread.html)

1个进程中可以开启多条线程，每条线程可以并行（同时）执行不同的任务。多线程技术可以提高程序的执行效率。CPU只能处理1条线程，只有一条线程在工作。同一时间内，CPU只能处理1条线程，只有1条线程在工作（执行）;多线程并发（同时）执行，其实是CPU快速地在多条线程之间调度（切换）。如果CPU调度线程的时间足够快，就造成了多线程并发执行的假象。CPU会在N多线程之间调度，CPU会类似，消耗大量的CPU资源；每条线程被调度执行的频次会较低（线程的执行效率减低）。一般开到3-5条线程。

#### C++11 thread

* thread 构造函数 (当你创建了一个（非空的）线程对象时，对应线程就会执行，不需要显式的调用start或者run)
```cpp
...
join()|调用了这个函数的线程对象，一定要等这个线程对象的方法（在构造时传入的方法）执行完毕后（或者理解为这个线程的活干完了！），这个join()函数才能得到返回
joinable()|检查线程是否可被 join, 通常不能被joinable有以下几种情况：1 由thread的缺省构造函数而造成的(thread()没有参数), 2 该thread被move过(包括move构造和move赋值), 3 该线程被join或者detach过
detach()|Detach 线程, 将当前线程对象所代表的执行实例与该线程对象分离，使得线程的执行可以单独进行。一旦线程执行完毕，它所分配的资源将会被释放。调用 detach 函数之后：*this 不再代表任何的线程执行实例, joinable() == false, get_id() == std::thread::id()。另外，如果出错或者 joinable() == false，则会抛出 std::system_error
...
sleep_for()|线程休眠某个指定的时间片(time span)，该线程才被重新唤醒，不过由于线程调度等原因，实际休眠时间可能更长

#### [mutex,锁管理类](https://blog.csdn.net/chenxun_2010/article/details/49786263)
...
[STL 方法含义索引](https://gitee.com/wlf621/Interview/tree/master/STL)

### STL 容器

容器 | 底层数据结构 | 时间复杂度 | 有无序 | 可不可重复 | 其他
---|---|---|---|---|---
[array](https://gitee.com/wlf621/Interview/tree/master/STL#array)|数组|随机读改 O(1)|无序|可重复|支持随机访问
[vector](https://gitee.com/wlf621/Interview/tree/master/STL#vector)|数组|随机读改、尾部插入、尾部删除 O(1)<br/>头部插入、头部删除 O(n)|无序|可重复|支持随机访问
[deque](https://gitee.com/wlf621/Interview/tree/master/STL#deque)|双端队列|头尾插入、头尾删除 O(1)|无序|可重复|一个中央控制器 + 多个缓冲区，支持首尾快速增删，支持随机访问
[forward_list](https://gitee.com/wlf621/Interview/tree/master/STL#forward_list)|单向链表|插入、删除 O(1)|无序|可重复|不支持随机访问
[list](https://gitee.com/wlf621/Interview/tree/master/STL#list)|双向链表|插入、删除 O(1)|无序|可重复|不支持随机访问
[stack](https://gitee.com/wlf621/Interview/tree/master/STL#stack)|deque / list|顶部插入、顶部删除 O(1)|无序|可重复|deque 或 list 封闭头端开口，不用 vector 的原因应该是容量大小有限制，扩容耗时
[queue](https://gitee.com/wlf621/Interview/tree/master/STL#queue)|deque / list|尾部插入、头部删除 O(1)|无序|可重复|deque 或 list 封闭头端开口，不用 vector 的原因应该是容量大小有限制，扩容耗时
[priority_queue](https://gitee.com/wlf621/Interview/tree/master/STL#priority_queue)|vector + max-heap|插入、删除 O(log<sub>2</sub>n)|有序|可重复|vector容器+heap处理规则
[set](https://gitee.com/wlf621/Interview/tree/master/STL#set)|红黑树|插入、删除、查找 O(log<sub>2</sub>n)|有序|不可重复|
[multiset](https://gitee.com/wlf621/Interview/tree/master/STL#multiset)|红黑树|插入、删除、查找 O(log<sub>2</sub>n)|有序|可重复|
[map](https://gitee.com/wlf621/Interview/tree/master/STL#map)|红黑树|插入、删除、查找 O(log<sub>2</sub>n)|有序|不可重复|
[multimap](https://gitee.com/wlf621/Interview/tree/master/STL#multimap)|红黑树|插入、删除、查找 O(log<sub>2</sub>n)|有序|可重复|
[unordered_set](https://gitee.com/wlf621/Interview/tree/master/STL#unordered_set)|哈希表|插入、删除、查找 O(1) 最差 O(n)|无序|不可重复|
[unordered_multiset](https://gitee.com/wlf621/Interview/tree/master/STL#unordered_multiset)|哈希表|插入、删除、查找 O(1) 最差 O(n)|无序|可重复|
[unordered_map](https://gitee.com/wlf621/Interview/tree/master/STL#unordered_map)|哈希表|插入、删除、查找 O(1) 最差 O(n)|无序|不可重复|
[unordered_multimap](https://gitee.com/wlf621/Interview/tree/master/STL#unordered_multimap)|哈希表|插入、删除、查找 O(1) 最差 O(n)|无序|可重复|
...
#### [构造方法](https://www.cnblogs.com/gj-Acit/archive/2013/05/06/3062628.html)
...
---|---|---|---|---
[冒泡排序](https://gitee.com/wlf621/Interview/tree/master/Algorithm/BubbleSort.h) | O(n<sup>2</sup>)|O(n<sup>2</sup>)|O(1)|稳定
[选择排序](https://gitee.com/wlf621/Interview/tree/master/Algorithm/SelectionSort.h) | O(n<sup>2</sup>)|O(n<sup>2</sup>)|O(1)|数组不稳定、链表稳定
[插入排序](https://gitee.com/wlf621/Interview/tree/master/Algorithm/InsertSort.h) | O(n<sup>2</sup>)|O(n<sup>2</sup>)|O(1)|稳定
[快速排序](https://gitee.com/wlf621/Interview/tree/master/Algorithm/QuickSort.h) | O(n*log<sub>2</sub>n) |  O(n<sup>2</sup>) | O(log<sub>2</sub>n) | 不稳定
[堆排序](https://gitee.com/wlf621/Interview/tree/master/Algorithm/HeapSort.cpp) | O(n*log<sub>2</sub>n)|O(n*log<sub>2</sub>n)|O(1)|不稳定
[归并排序](https://gitee.com/wlf621/Interview/tree/master/Algorithm/MergeSort.h) | O(n*log<sub>2</sub>n) | O(n*log<sub>2</sub>n)|O(n)|稳定
[希尔排序](https://gitee.com/wlf621/Interview/tree/master/Algorithm/ShellSort.h) | O(n*log<sup>2</sup>n)|O(n<sup>2</sup>)|O(1)|不稳定
[计数排序](https://gitee.com/wlf621/Interview/tree/master/Algorithm/CountSort.cpp) | O(n+m)|O(n+m)|O(n+m)|稳定
[桶排序](https://gitee.com/wlf621/Interview/tree/master/Algorithm/BucketSort.cpp) | O(n)|O(n)|O(m)|稳定
[基数排序](https://gitee.com/wlf621/Interview/tree/master/Algorithm/RadixSort.h) | O(k*n)|O(n<sup>2</sup>)| |稳定
...
---|---|---|---
[顺序查找](https://gitee.com/wlf621/Interview/tree/master/Algorithm/SequentialSearch.h) | O(n) | O(1) | 无序或有序
[二分查找（折半查找）](https://gitee.com/wlf621/Interview/tree/master/Algorithm/BinarySearch.h) | O(log<sub>2</sub>n)| O(1) | 有序
[插值查找](https://gitee.com/wlf621/Interview/tree/master/Algorithm/InsertionSearch.h) | O(log<sub>2</sub>(log<sub>2</sub>n)) | O(1) | 有序
[斐波那契查找](https://gitee.com/wlf621/Interview/tree/master/Algorithm/FibonacciSearch.cpp) | O(log<sub>2</sub>n) | O(1) | 有序
[哈希查找](https://gitee.com/wlf621/Interview/tree/master/DataStructure/HashTable.cpp) | O(1) | O(n) | 无序或有序
[二叉查找树（二叉搜索树查找）](https://gitee.com/wlf621/Interview/tree/master/Algorithm/BSTSearch.h) |O(log<sub>2</sub>n) |   | 
[红黑树](https://gitee.com/wlf621/Interview/tree/master/DataStructure/RedBlackTree.cpp) |O(log<sub>2</sub>n) | |
...
算法 |思想| 应用
---|---|---
[分治法](https://zh.wikipedia.org/wiki/%E5%88%86%E6%B2%BB%E6%B3%95)|把一个复杂的问题分成两个或更多的相同或相似的子问题，直到最后子问题可以简单的直接求解，原问题的解即子问题的解的合并|[循环赛日程安排问题](https://gitee.com/wlf621/Interview/tree/master/Problems/RoundRobinProblem)、排序算法（快速排序、归并排序）
[动态规划](https://zh.wikipedia.org/wiki/%E5%8A%A8%E6%80%81%E8%A7%84%E5%88%92)|通过把原问题分解为相对简单的子问题的方式求解复杂问题的方法，适用于有重叠子问题和最优子结构性质的问题|[背包问题](https://gitee.com/wlf621/Interview/tree/master/Problems/KnapsackProblem)、斐波那契数列
...
* [管道（PIPE）](https://blog.csdn.net/yishizuofei/article/details/78301414)
...
* [共享内存（Shared Memory）](https://blog.csdn.net/ypt523/article/details/79958188)：映射一段能被其他进程所访问的内存，这段共享内存由一个进程创建，但多个进程都可以访问
    * 优点：无须复制，快捷，信息量大
    * 缺点：
...
* 锁机制：[包括互斥锁/量（mutex）](https://blog.csdn.net/chenxun_2010/article/details/49786263)、[读写锁（reader-writer lock）](https://blog.csdn.net/gongjianbo1992/article/details/100061344)、自旋锁（spin lock）、[条件变量（condition）](https://blog.csdn.net/fanyun_01/article/details/102916793)
...
### [页面置换算法](https://blog.csdn.net/weixin_41705703/article/details/90608445)
...
### [Linux常用命令](https://blog.csdn.net/tianzongnihao/article/details/80539264)
...
> 如：`https://gitee.com/wlf621/Interview#cc`
...
### [并发控制](https://www.cnblogs.com/imcati/p/9782924.html)
...
* [牛客网 . 2021届校招 | 2020 IT名企校招日程](https://www.nowcoder.com/school/schedule)
...
#### [deque的底层实现](https://www.cnblogs.com/lfri/p/12565031.html)
...
在c语言的编译器下，编译器将函数名编译为”_函数名”，重载之后，编译器将所有重载的函数编译为“_函数名”形式，而在c++编译器下，将函数名编译为“？@函数名@@yahhh@Z”的形式->函数名+返回值类型+参数类型，所以不会形成相同名字的函数，因此c语言中没有函数重载。
...
#### [如果线上某台虚机CPU Load过高，该如何快速排查原因？只介绍思路和涉及的Linux命令即可](https://www.jianshu.com/p/6d573e42310a)
...
<a href="https://gitee.com/wlf621/Interview/graphs/contributors"><img src="https://opencollective.com/interview/contributors.svg?button=false" /></a>
...
[![CC BY-NC-SA 4.0](https://i.creativecommons.org/l/by-nc-sa/4.0/88x31.png)](https://gitee.com/wlf621/Interview/blob/master/LICENSE)
